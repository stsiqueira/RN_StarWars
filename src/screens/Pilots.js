...
const Pilots = () => {
  ...
  const { savePilots, saveStarships } = bindActionCreators(
    actionCreators,
    dispatch
  );
  ...
  const FilterBySearch = () => {
    ...
    searchArray.map((searchString) => {
      ...
      pilots.map((pilot) => {
        if (
          pilot.name.toLowerCase().includes(searchString.toLowerCase()) ||
      ...
      searchResult.map(
        (pilotUrl) =>
          (newPilots = [
            ...newPilots,
            pilots.find((pilot) => pilot.url === pilotUrl),
          ])
      );
    ...
    <ScrollView>
      ...
      >
        <TextInput
          ...
          style={{ borderBottomWidth: 1, width: "70%", marginTop: 16 }}
        />
